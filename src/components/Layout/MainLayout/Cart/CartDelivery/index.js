...
function CartDelivery() {
    ...
    const [customerPhoneNumberError, setCustomerPhoneNumberError] =
        useState("");
    ...
    const handleSelectAddressChange = async (value) => {
        if (value.value) {
            try {
                ...
                const distance =
                    response.data?.routes?.car?.distance?.value / 1000 || 0;
                ...
                if (data.distance > defaultShippingMethod.initialDistance) {
                    data.shippingCost =
                        ...
                        (Math.ceil(data.distance) -
                            defaultShippingMethod.initialDistance) *
                            defaultShippingMethod.additionalCharge;
                } else {
            ...
            >
                ...
                >
                    ...
                    <span className={cx("font-primary", "fs-16")}>
                        {currencyConvert(cartInfor.itemsTotal)}
                    </span>
                </div>
                <div
                    className={cx(
                ...
                >
                    ...
                    >
                        Phí vận chuyển{" "}
                        {cartInfor.distance > 0 &&
                            cartInfor.distance?.toFixed(2) + "km"}
                    </span>
                    <span className={cx("font-primary", "fs-16")}>
                        {cartInfor.shippingCost
                            ? currencyConvert(cartInfor.shippingCost)
                            : "Tính phí vận chuyển"}
                    </span>
                </div>
                ...
                {cartInfor.shippingCost ? (
                    ...
                    >
                        ...
                        <span className={cx("font-primary", "fs-16")}>
                            {currencyConvert(
                                cartInfor.itemsTotal + cartInfor.shippingCost
                            )}
                        </span>
                    </div>
                ) : (
